...
`;
		...
		const ZH_DICT = {
			...
			"passkeys.insecure": "通行密钥需要 HTTPS：这可能是当前地址是明文 HTTP，也可能是反向代理没有把 HTTPS 传进来（需保留 Host 并发送 X-Forwarded-Proto: https，且其地址在 trustedProxyAddresses 中）。",
		...
		const EN_DICT = {
			...
			"passkeys.insecure": "Passkeys need HTTPS: either this address is plain HTTP, or the reverse proxy does not pass HTTPS through (it must keep the public Host and send X-Forwarded-Proto: https from a trustedProxyAddresses entry).",
