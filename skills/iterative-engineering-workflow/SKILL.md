...
The `agent()` schema is a strict object-rooted JSON subset: the root must declare `type: "object"`. At each node, `enum` and `const` require a compatible scalar `type`, object keywords (`properties`, `required`, boolean `additionalProperties`) require `type: "object"`, and `items` requires `type: "array"`. A nested `oneOf` must contain at least two schemas and cannot share its node with `type` or those constraint keywords; place typed constraints inside its branches. For example, use `{ type: "string", enum: ["pass", "fail"] }`, never `{ enum: ["pass", "fail"] }`. Use only the tool's advertised keywords and plain JSON values. Preflight every generated result schema recursively before launch (see the controller reference), pass it as `opts.schema`, and validate result semantics before changing state.

Within the script, `agent(prompt, opts)` accepts only `label`, `phase`, `schema`, `provider`, and `model` options. Keep all other policy data inside `args` or the controller's plain JSON state; do not extend the hook's options object.
...
Resolve an unassigned low-cost role to `models.default`, and require that default and all low-cost overrides to be confirmed low-cost routes. Require every high-capability role to have an explicit route; a missing high-capability route is `blocked` rather than silently downgraded. Declare disjoint `modelPolicy.lowCostRoles` and `modelPolicy.highCapabilityRoles` explicitly; count calls by role after the parent verifies the resolved provider/model classifications. Freeze routes and policy at launch. Use the low-cost route for planning, implementation, reconciliation, read-only audits, first-pass review, and verification. Reserve the high-capability route for a selective semantic review or an escalation trigger: ambiguous acceptance, security/data/concurrency risk, a required check that remains failed after one repair, a repeated finding, or an ownership/scope conflict. Use actual configured provider/model ids. A low-risk task may finish without a high-capability call when deterministic checks and the declared acceptance criteria provide sufficient evidence.

Keep a hard `maxHighCapabilityCalls` budget. Count prechecks, planners, audits, reconciliation, verification, and null results in the total budget; reserve enough calls for reconciliation and final verification before admitting a writer. Child prompts must prohibit further delegation and model switching, since descendant calls are outside these controller counters. Record every high-capability call in `modelUsage`; a null result still consumes the budget and requires workspace reconciliation. When the budget is exhausted, return `blocked` with the missing evidence instead of retrying the same route or silently substituting another high-cost call. A high-capability escalation receives authority to diagnose, implement the bounded repair, add regression tests, and verify its current-generation change in one call. This is the preferred handoff for a difficult issue; it prevents an expensive review from bouncing vague feedback back to a low-cost worker.

The default allowance is at most one combined high-capability gate when triggered or explicitly required: it reviews the final evidence and directly fixes a bounded blocker when necessary. A parent may explicitly raise the limit to two for high-risk work, but the controller never increases it automatically.
...
Keep `reviewer` in `lowCostRoles` for the first pass. Move it to `highCapabilityRoles` only when a read-only high-capability review is explicitly required; otherwise use `escalator` as the combined review-and-repair role.

## Input policy

Build `args` from the user's request and repository facts. Enable simplification only when the user's intent includes it; materially missing acceptance criteria block write dispatch until clarified.
...
For a large task, keep the workflow call's control plane small. Put the full goal, repository paths and branches, constraints, acceptance criteria with stable ids, stage configuration, ownership map, and risk notes in one immutable manifest file created before launch. Pass only `manifestPath`, `manifestDigest`, `specRevision`, model routes, `modelPolicy`, and `limits` through `args`; the controller reference defines digest checks and compact precheck output. Pass the manifest path and digest to every subagent. In manifest mode, the state and prompts below use criterion/task ids, bounded summaries, and artifact paths plus digests in place of the full goal, constraints, acceptance text, diffs, and reports. The manifest is the frozen contract, not a reason to load its full contents into workflow JSON.
