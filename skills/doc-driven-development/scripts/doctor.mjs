...
runCLI(() => {
  ...
  if (args.flags.help) {
    console.log('Usage: node doctor.mjs [project] [--json] [--strict] [--probe]\n'
      + 'Read-only installation/entry checks. Never invokes an AI or edits any project file.\n'
  ...
  if (args.flags.json) console.log(JSON.stringify(report, null, 2));
    ...
    console.log(`Static installation: ${report.static}; agent runtime: ${report.runtime}.`);
    ...
    console.log('No AI was invoked. This is not a semantic, approval, or enforcement guarantee.');
