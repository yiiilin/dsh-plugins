...
runCLI(() => {
  ...
  if (!args.flags['no-tests']) {
    const result = spawnSync(process.execPath, ['--test', '--test-reporter=tap', path.join(root, 'tests/run.mjs'), path.join(root, 'tests/install.mjs'), path.join(root, 'tests/activation.mjs'), path.join(root, 'tests/layout.mjs'), path.join(root, 'tests/design.mjs'), path.join(root, 'tests/closure.mjs'), path.join(root, 'tests/context.mjs')], {
