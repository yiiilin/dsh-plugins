...
import { itemHeadings, canonicalItemLines } from './identity.mjs';
...
export function evidenceSections(text) {
  const parsed = itemHeadings(text), scan = { ...parsed.scan, headings: parsed.headings }, found = [];
  for (let i = 0; i < scan.headings.length; i++) {
    const h = scan.headings[i], id = h.id?.startsWith('E-') ? h.id : null;
    if (!id) continue;
...
export function contractHash(doc) {
  ...
  const lines = canonicalItemLines(doc.text).slice(first).filter((_, i) => !bookkeepingHeadings.has(i + first) && !sections.some(s => i + first >= s.start && i + first < s.end));
