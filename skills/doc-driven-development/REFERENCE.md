...
观察可以经真实确认直接采纳，不必等待第一次代码变更。已有对同一范围/含义的真实批准时应主动复用并维护字段，不要求用户为了把 observed 改 accepted 再确认一次；只有代码/测试事实仍不构成批准。采纳部分规则时，将获批范围独立成当前文档或短提案；其余观察继续保留，不用一个 `accepted` 覆盖未获批章节。
...
先查明事实、发现缺口，与人推演重要场景，再整理方案与待裁决选择；不先把未经讨论的决定写成既定事实。已确认文档有实现约束力，不具有不可质疑性。发现矛盾时明确指出，保留现行基线，通过讨论改变它。每项 `D-<主题>-<编号>` 包含场景、问题、真实选项、推荐理由、代价、适用边界/例外、影响范围、确认来源、是否授权实施及剩余未决项。本轮明确确认/撤回/范围变化后即记录，不等整场讨论结束。内部标识不重排、不复用；对人用自然主题名称，正文标题可用隐藏 ddd:item 绑定，见 [COMMUNICATION.md](COMMUNICATION.md)。
...
[EXECUTION.md](EXECUTION.md) 将原始实施任务与具体设计确认联用，不再要求逐阶段重复批准同一方案。仅设计/暂停不自动实施；新增重要选择仍回讨论。把既有真实批准写入 Approval 是记录维护，不是一次新决策；observed 不能靠搜索/测试自动获批。
