...
适用版本：0.2.5。工具需要 Node.js 22+，只用标准库，不联网、不调用模型、不执行项目脚本。
...
对没有安装收据的 v0.2.0/v0.2.1/v0.2.2/v0.2.3/v0.2.4，发行包内包含原发行文件哈希：全部旧发行文件仍与原版一致时，可以直接升级到 0.2.5（只迁移工具归属，不迁移项目设计）。旧目录里额外的个人文件保留；它们与新版本文件重名时停止。修改过、缺文件或无法识别的旧包不会覆盖；先备份到宿主技能发现范围之外并人工对比。

本版配置格式与 v0.2.0/v0.2.1 相同，原来渐进/全量接管记录继续沿用。`.doc-driven/` 是新增加的安装元数据区域，不进入业务代码快照或全量阅读分母。

### 修复历史 config-only 半安装

`.doc-driven.json` 为 `enabled: true`、项目没有 skill 目录和入口区块时，不删除或重新生成配置。使用新解压 v0.2.5 的同一 `install` 命令预览和执行；它保留配置、原有 docs 及规则正文，仅补齐缺失的工具接入和索引区块。已有配置内的目录、语言、模式继续生效。
...
从项目外新包运行 install 预览/--apply；不要先覆盖复制旧技能目录。内置未修改 v0.2.3 发行文件哈希，同时保留更早已支持版本。受管规则块会更新为即时记录、场景优先解释、按风险评审、上下文刷新和双向版本证据要求；原区块之外文字保持不变。个人修改过的包文件/区块仍拒绝自动覆盖，既有设计正文及 JSON 配置不重写、不迁移、不自动把旧证据补成新通过。

新增 context.mjs 默认只读；只有显式 --out 才创建新 `.doc-driven/context/*.json`，快照无论安装/升级/卸载都不当发行文件删除。doctor --probe 增加本轮已确认如何保存、陌生术语怎样解释、评审如何裁决以及旧证据失效的提问；仍不实际调用模型、runtime 仍需真实新会话验证。

## v0.2.5：明确的 agent 升级入口

升级请求首先读取 [UPGRADE.md](UPGRADE.md)，从项目外新完整发行包运行 `scripts/upgrade.mjs <project>` 预览，授权覆盖且无冲突时带 `--apply` 执行。它复用安全 install；不新增数据库、不下载未知包、不把旧目录内运行命令说成更新。增加旧版本拒绝降级，支持未修改 v0.2.4 的识别。结果区分源/已装版本、原规范保留、项目正文未迁移和运行会话待重读。

本版入口加入持续实施授权、自然名称报告、真实入口验收及可选两种多 agent 分工。索引生成区块显示自然标题；人工区块保留。项目设计不自动移除 ID、不批量重绑定 E、不改 observed/accepted，不擅自启动服务/迁移/清理卷。
