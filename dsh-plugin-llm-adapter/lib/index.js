...
const SERVICE_TIERS = Object.keys({
	auto: true,
	default: true,
	flex: true,
	scale: true,
...
/** Add a model-specific OpenAI Responses service tier when configured. */
function serviceTierOf(model) {
	if (model?.api !== "openai-responses") return void 0;
	const tier = model?.serviceTier;
	return typeof tier === "string" && SERVICE_TIERS.includes(tier) ? tier : void 0;
}

/** Apply a model's Responses service tier while preserving caller payload hooks. */
function withServiceTier(options, model) {
	const serviceTier = serviceTierOf(model);
	if (serviceTier === void 0) return options;
	const callerOnPayload = options?.onPayload;
	return {
		...(options ?? {}),
		serviceTier,
		onPayload: async (payload, receivedModel) => {
			const next = callerOnPayload === void 0 ? payload : await callerOnPayload(payload, receivedModel);
			const base = next === void 0 ? payload : next;
...
/** Wrap both provider entry points; pi-ai's simple path otherwise drops serviceTier. */
function withModelRequestOptions(base) {
	return {
		stream: (model, context, options) => base.stream(model, context, withServiceTier(options, model)),
		streamSimple: (model, context, options) => {
			if (serviceTierOf(model) === void 0) return base.streamSimple(model, context, options);
			const baseOptions = buildBaseOptions(model, context, options, options?.apiKey);
			const clampedReasoning = options?.reasoning ? clampThinkingLevel(model, options.reasoning) : void 0;
			return base.stream(model, context, withServiceTier({
				...baseOptions,
...
function buildProvider(spec) {
	...
	return createProvider({
		...
		models: spec.models,
		api: withModelRequestOptions(factory())
	});
}
//#endregion
...
/**
* Split the system prompt out of the request options.
*
...
var PiAiAdapter = class extends LlmAdapter {
	...
	*/
	...
	modelInfo(snapshot, provider, model) {
		...
		const defaultLevel = describableReasoningLevel(resolvedModel, resolvedModel.reasoningEffort ?? profile.reasoning);
	...
	async *streamWithSnapshot(options, snapshot) {
		...
		try {
			...
			const reasoning = resolveReasoningLevel(model, options.reasoningEffort ?? model.reasoningEffort ?? profile.reasoning);
...
*
* `anthropic-messages` is the exception: its public listing lives beside the
* `/v1` message endpoint rather than under it, so the base is rewritten
* instead of extended, and the page size is pinned to the one page this reads.
*/
...
* @param storedProfile - the Host-owned inputs for the named route, asked for
*   only when the draft carries none and only on the path that reaches the
*   network: the credential the route already stored plus its configured
*   headers. A configuration surface never holds a stored secret — it edits a
*   redacted descriptor — so without this an already-configured route would be
*   interrogated unauthenticated and answer 401, and a route that authenticates
*   with a custom header would lose that header on the probe.
...
function apply(ctx, config) {
	...
	*/
	...
	/**
	* Host-owned request inputs for discovery of one configured route. The
	* stored headers ride along so a route that authenticates with a custom
	* header (rather than a bearer token) is probed the same way a real request
	* would be; the key resolves lazily so an unconfigured route still probes
	* unauthenticated instead of failing before the fetch.
	*/
